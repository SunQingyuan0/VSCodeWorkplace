...
void Dijkstra(GraphMatrix elem, int t, visit* data)
{
    data[t].lowcost = 0;
    data[t].parent = -1;
    for (int i = 0; i < elem.vexnum; i++)
    {
        ...
        for (int j = 0; j < elem.vexnum; j++)
        {
            ...
            {
                if (elem.arc[p][j].vexadj+data[p].lowcost < data[j].lowcost)
                {
                    data[j].lowcost = elem.arc[p][j].vexadj+data[p].lowcost;
                    data[j].parent = p;
                }
            }
        }
